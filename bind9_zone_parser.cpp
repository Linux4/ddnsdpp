...
long bind9_zone_parser::get_serial() {
    ...
    while (getline(f, tmp, '\n')) {
        ...
        /*
        if (broken.size() > 5 && (broken.at(4) == "SOA" || broken.at(5) == "SOA")) {
            for (unsigned long i = 5; i < broken.size(); i++) {
                try {
                    return stol(broken.at(i));
                } catch (std::invalid_argument &e) {
                    if (broken.at(i).find(";") != std::string::npos) { // comment
                        break;
                    }
        ...
        if (tmp.find("(") != std::string::npos) { // multiline entry
            getline(f,
        }
    }*/
